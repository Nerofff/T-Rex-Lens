...
def run_model(image_path):
    ...
    for key, value in label_mapping.items():
        print(f"{key}: {value}")
    # In ra giá trị của vectơ dự đoán và xác suất của từng lớp
    print("Vector dự đoán:", prediction)
    for i, value in enumerate(prediction[0]):
        print(f"Xác suất của lớp {i}: {value}")
...
def predict():
    ...
    if os.path.exists(image_path):
        ...
        google_link = f'https://www.google.com.vn/search?tbm=isch&q={result}'
